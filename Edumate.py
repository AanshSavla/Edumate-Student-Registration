...
def student_new_record(): 
    ...
    Label(screen4, text="", bg='#174873', width='70', height='18').place(x=0, y=127) 
...
def enter_new_record(entryField, semester): 
    ...
    if found == 0: 
        if semester.get() == '--0--': 
            ...
            # enter new record 
            connection = pymysql.connect(host="localhost", port=3308,user="root", passwd="", database="edumate") # database connection 
...
def student_records(): 
    ...
    Label(screen3, text="", bg='#174873', width='80', height='22').place(x=0, y=127) 
...
def welcome_page(student_info):
     ...
     Message(screen2, text='" Some people dream of accomplishing great things. Others stayawake and make it happen. "\n\n - By Some Night Owl', width='180', font=("Helvetica",10, 'bold', 'italic'), fg='white', bg='#174873', anchor = CENTER).place(x=10, y=100)
     #photo = PhotoImage(file="global.jpeg") # opening left side image - Note: Ifimage is in same folder then no need to mention the full path
     #label = Label(screen2, image=photo, text="") # attaching image to the label
     #label.place(x=10, y=270)
     #label.image = photo # it is necessary in Tkinter to keep a instance of image to displayimage in label
     #photo1 = PhotoImage(file="global.jpeg") # opening right side image - Note: Ifimage is in same folder then no need to mention the full path
     #label1 = Label(screen2, image=photo1, text="") # attaching image to the label
     #label1.place(x=200, y=96)
     #label1.image = photo1 # it is necessary in Tkinter to keep a instance of image to displayimage in label
...
# registration window
def register():
     global screen1, fullname, email, password, repassword, university, gender, tnc # makingall entry field variable global
     fullname = StringVar()
     email = StringVar()
     password = StringVar()
     ...
     Label(screen1, text="", bg='#174873', width='70', height='24').place(x=45, y=120)
...
def login_verify():
     ...
     connection = pymysql.connect(host="localhost", port=3308,user="root", passwd="",database="edumate") # database connection
     cursor = connection.cursor()
     select_query = "SELECT * FROM student_details where email = '" +username_verify.get() + "' AND password = '" + password_verify.get() + "';" # queries forretrieving values
     cursor.execute(select_query) # executing the queries
     student_info = cursor.fetchall()
     connection.commit() # commiting the connection then closing it.
     connection.close() # closing the connection of the database
     if student_info:
         messagebox.showinfo("Congratulation", "Login Succesfull") # displaying message forsuccessful login
...
def main_screen():
     ...
     Label(screen, text="", bg='#174873',width='70', height='17').place(x=45, y=120) # bluebackground in middle of window
